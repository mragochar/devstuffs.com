<p align="center">
  <img src="https://devstuffs.com/logo-dark.svg" alt="devstuffs" height="120px"/>
</p>

<p align="center"><b>Learn DevOps, Linux and More</b></p>

<p align="center">
  <a href="https://devstuffs.com">Start Learning</a> · <a href="https://devstuffs.com/blog">Read Blogs</a> . <a href="https://discord.gg/d6SEtHjg2u">Join Discord</a>
</p>

🚀 [devstuffs](https://devstuffs.com) is a set of hands-on courses for learning Linux, shell scripting, Docker, Ansible, and more DevOps concepts as they get written baked out straight from my learning experiences.

👉 If you are new to Linux and DevOps, start here: [Linux Fundamentals](https://devstuffs.com/linux) 🐧

---

## 🧱 Fundamentals Courses

- [Linux Fundamentals](https://devstuffs.com/linux) 🐧
- [Shell Scripting](https://devstuffs.com/shell-scripting) ⬛>_🖳
- [Git and Github](https://devstuffs.com/git) 𖦥🗁

## ♾️ DevOps Specific Courses

- [Docker](https://devstuffs.com/docker) 🐳
- [Ansible](https://devstuffs.com/ansible) Ⓐ

## 🔥Upcoming Hot ☕︎ Courses 💥

- [Kubernetes](https://devstuffs.com) ☸️
- [Terraform](https://devstuffs.com) 🛠️

## 🆓 Curated List of Free Courses 🌟

- [✨ Free Ansible Ⓐ Course](https://github.com/mragochar/free-ansible-course)

## 🖋️✍🏻 Contribute to devstuffs.com

You can contribute as a guest writing DevOps and Linux related blog post. If something you send in gets written up and published, **you’re credited as a contributor** on that post, with your name, image and a link to your profile.

**✉️ Mail** personally ✉︎: `hello@devstuffs.com`  
If you wish to know more about *devstuffs* and contributing, see [About devstuffs](https://devstuffs.com/about).

## 🚀 devstuffs.com Keeps Growning

The goal of [devstuffs.com](https://devstuffs.com) is to keep growing. Support by start learning:

![](/images/tags.png)

## 💬Get in Touch 🙋🏻‍♂️

**Email ✉︎**:
hello@devstuffs.com
(Preferred for inquiry 🙋🏻‍♂️ and contributing 🤝)

**Discord ➤**:
https://discord.gg/d6SEtHjg2u
(Preferred for active discissions 🗣️)

Corrections, blog pitches, or just questions about a course — either channel works.
